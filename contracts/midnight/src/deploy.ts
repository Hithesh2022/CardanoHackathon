...
const waitForFunds = (wallet: Wallet) =>
  Rx.firstValueFrom(
    wallet.state().pipe(
      ...
      Rx.tap((balance: any) => console.log(`Wallet funded with balance: ${balance}`))
    )
  );

async function main() {
  console.log("🌙 AtlasCred Midnight Deployment\n");
  console.log("Deploying credit score ZK proof contract to Midnight Testnet...\n");
  ...
  try {
    ...
    const ScoreProofModule = await import(contractModulePath);
    const contractInstance = new ScoreProofModule.Contract({});
    ...
    const zkConfigPath = path.join(contractPath, "managed", "score-proof");
    const providers = {
      privateStateProvider: levelPrivateStateProvider({
        privateStateStoreName: "score-proof-state"
      }),
    ...
    const deployed = await deployContract(providers, {
      ...
      privateStateId: "scoreProofState",
      initialPrivateState: {}
    });
    ...
    const info = {
      ...
      walletAddress: (state as any).address
    };
